...
### 2026-02-05 팝업 관리 시스템 추가
- **관리자 팝업 CRUD**: 공지사항 팝업 생성/수정/삭제/활성화 토글
- **스케줄링**: 시작일/종료일 설정으로 자동 노출 제어
- **페이지 타겟팅**: 전체(all), 랜딩, 대시보드, 플레이스 리뷰 페이지별 노출
- **우선순위**: 숫자로 팝업 표시 순서 제어 (높을수록 먼저 표시)
- **사용자 선택 옵션**: 
  - "오늘 하루 안 보기" 버튼 표시 여부
  - "다시 보지 않기" 버튼 표시 여부
- **localStorage 기반 dismiss**: 사용자별 팝업 숨김 상태 저장
- **관련 파일**:
  - shared/schema.ts: popups 테이블 스키마
  - server/admin-routes.ts: 관리자 팝업 API
  - server/routes.ts: 사용자용 활성 팝업 조회 API
  - client/src/pages/admin/PopupsTab.tsx: 관리자 UI
  - client/src/components/popup-modal.tsx: 사용자용 팝업 모달
...
- types.ts에 API_TYPE_LABELS/COLORS 중앙 관리
- UserDetailModal에서 중복 레이블 제거, 공용 레이블 재사용

### 2026-01-29 관리자 콘솔 UI/UX 간소화
- **시스템 API 키 탭**: 7개 요약 카드 → 2개 컴팩트 카드 (검색 API/데이터랩 API)
