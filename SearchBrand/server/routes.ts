...
import {
  getKeywordVolume,
  isConfigured as isNaverAdConfigured,
  type KeywordVolumeResult,
} from "./naver-ad-api";
import {
  getKeywordTrend,
  isConfigured as isDataLabConfigured,
...
import placeReviewRoutes, {
  initPlaceReviewWorker,
} from "./place-review-routes";
import feedbackRoutes from "./feedback-routes";
import {
  getAllServicesStatus,
  getQuickRedisStatus,
  getQuickChromeStatus,
} from "./services/service-status";
import {
  getAvailableSystemApiKey,
...
const apiKeyLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: {
    message: "API 키 관련 요청이 너무 많습니다. 잠시 후 다시 시도하세요.",
  },
  keyGenerator: generateRateLimitKey,
  validate: { xForwardedForHeader: false },
});
...
const searchQuerySchema = z.object({
  keyword: z
    .string()
    .min(1, "키워드는 필수입니다")
    .max(100, "키워드는 100자 이하여야 합니다")
    .transform((s) => s.trim()),
  sort: z.enum(["sim", "date"]).default("sim"),
  page: z
    .string()
    .regex(/^\d+$/, "페이지 번호는 숫자여야 합니다")
    .default("1")
    .transform((s) => Math.max(1, Math.min(100, parseInt(s, 10)))),
});

const channelSearchQuerySchema = searchQuerySchema.extend({
  channel: z.enum(["blog", "cafe", "kin", "news"], {
    message: "유효한 채널이 아닙니다 (blog, cafe, kin, news)",
  }),
});
...
export async function registerRoutes(
  ...
  initPlaceReviewWorker().catch((error) => {
    console.log(
      "[PlaceReview] Worker not started (Redis may not be running):",
      error,
    );
  });

  // Threads 캠페인 리다이렉트 라우트
  const THREADS_UTM_BASE =
    "/?utm_source=threads&utm_medium=social&utm_campaign=260205_traffic&utm_term=all";
  ...
  // iBoss 커뮤니티 리다이렉트
  app.get("/glitzy", (_req, res) => {
    res.redirect(
      302,
      "/?utm_source=iboss&utm_medium=community&utm_campaign=260205_traffic&utm_term=all&utm_content=guide",
    );
  });
    ...
    async (req: any, res) => {
      try {
        ...
        if (existing) {
          return res
            .status(400)
            .json({ message: "API key already exists. Use PUT to update." });
        }
      ...
      try {
        ...
        if (!credentials) {
          return res.status(503).json({
            message:
              "현재 사용 가능한 API 키가 없습니다. 관리자에게 문의하세요.",
        ...
        const [smartBlockResult, searchResult, volumeResult, trendResult] =
          await Promise.all([
            crawlPromise,
        ...
        storage
          .createSearchLog({ userId, searchType: "unified", keyword })
          .catch((err) => {
        ...
        if (error instanceof QuotaExceededError) {
          return res.status(429).json({
            message:
              "모든 API 키의 일일 한도가 소진되었습니다. 내일 다시 시도해주세요.",
            quota: error.quota,
          });
        }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const result = await searchSingleChannel(
          channel,
          keyword,
          sort,
          page,
  ...
  app.get("/api/popups/active", async (req: any, res) => {
    try {
      ...
      if (targetPage && targetPage !== "all") {
        conditions.push(eq(popups.targetPage, targetPage as string));
      }
