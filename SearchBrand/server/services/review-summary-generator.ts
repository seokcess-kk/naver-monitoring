...

{
  ...
  "suggestions": [
    "1. 구체적이고 실행 가능한 제안",
    "2. 키워드 기반 제안", 
    "3. 운영 또는 커뮤니케이션 관련 제안"
  ]
}`;

function buildUserPrompt(input: ReviewSummaryInput): string {
  const negativeKeywordsStr = input.topNegativeKeywords.length > 0
    ? input.topNegativeKeywords.map(k => `${k.keyword}(부정 ${k.negativeRatio}%)`).join(", ")
    : "없음";
  
  const positiveKeywordsStr = input.topPositiveKeywords.length > 0
...
function generateFallbackSummary(input: ReviewSummaryInput): ReviewSummaryResult {
  const { sentimentSummary, topNegativeKeywords, topPositiveKeywords, totalReviews } = input;
  ...
  if (topNegativeKeywords.length > 0) {
    suggestions.push(`"${topNegativeKeywords[0].keyword}" 관련 불만 개선 - 우선순위 높음`);
  }
  
  if (sentimentSummary.negative >= 20) {
    suggestions.push(`부정 리뷰 비율 ${sentimentSummary.negative}% - 고객 불만 요인 집중 분석 필요`);
  }
  
  if (topPositiveKeywords.length > 0) {
    suggestions.push(`"${topPositiveKeywords[0]}" 강점 유지 - 마케팅 메시지에 활용 권장`);
  }
...
export async function generateReviewSummary(input: ReviewSummaryInput): Promise<ReviewSummaryResult> {
  ...
  try {
    ...
    console.error("[ReviewSummaryGenerator] OpenAI failed, using fallback:", error);
    return generateFallbackSummary(input);
  }
}
