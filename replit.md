...
### 서비스 상태 모니터링 시스템 (1/21)
- 서비스 상태 API 추가 (`/api/services/status`, `/api/services/quick-status`)
- 사용자 알림: 서비스 불가 시 각 페이지에 경고 표시
  - 플레이스 리뷰: Redis 상태 확인
  - SOV 분석: OpenAI 상태 확인
  - 스마트블록: Chrome 상태 확인
- 관리자 대시보드: 시스템 상태 탭 추가
  - 전체 서비스 상태 한눈에 확인
  - 각 서비스별 상세 상태 및 영향 기능 표시
  - 30초마다 자동 갱신
- 상세 상태 API는 관리자 인증 필요
